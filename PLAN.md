...
- Validate and normalize control-plane mapping fields once before cache insertion: endpoint scheme/host/port policy, bucket, region, path prefix, addressing style, and credential presence.
...
| Status | Type | Item | Evidence / Gap |
| --- | --- | --- | --- |
| Incomplete | Work | 2A: Operation-specific request transforms | Gap: F2; `s3_proxy.go:179-185` treats every non-ListObjects request as an object path. |
| Incomplete | Work | 2B: Multipart namespace isolation | Missing: request and response transforms for ListMultipartUploads, CreateMultipartUpload, ListParts, and CompleteMultipartUpload. |
| Incomplete | Work | 2C: Error/redirect/header virtualization | Gap: upstream responses are copied unchanged except successful ListObjects XML and stale `Content-Length`. |
| Incomplete | Work | 2D: Proxy header/trailer sanitation | Missing: removal of `Connection`-nominated headers and an explicit trailer/authentication policy. |
| Incomplete | Work | 2E: Validated backend mapping | Gap: endpoint parse errors are ignored and mapping fields are trusted until the request hot path. |
| Incomplete | Test | 2F: Cross-tenant and no-leak integration suite | Missing: multipart, HeadBucket, redirect, and upstream-error coverage with real-identifier sentinels. |
| Incomplete | Test | 2G: Streaming and cancellation proof | Existing list tests cover streaming shape but not memory bounds, malformed XML, or cancellation across all transforms. |
| Incomplete | Gate | Complete virtual namespace | Missing: request containment and response non-disclosure evidence for every matrix row. |
